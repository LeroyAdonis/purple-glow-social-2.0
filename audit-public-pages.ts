...
async function auditPage(page: any, url: string, slug: string): Promise<PageResult> {
    console.log(`\n${'='.repeat(60)}`);
    ...
    const accessibilityObservations: string[] = [];
    ...
    const brokenImages = await page.evaluate(() => {
        ...
        return images
            .filter((img: any) => !img.complete || img.naturalHeight === 0)
            .map((img: any) => img.src || img.alt || 'unnamed image');
    });
    if (brokenImages.length > 0) {
        brokenImages.forEach((img: string) => {
    ...
    if (overflowingElements.length > 0) {
        overflowingElements.forEach((el: string) => {
    ...
    const imagesNoAlt = await page.evaluate(() => {
        ...
        return images
            ...
            .map((img: any) => img.src || img.outerHTML.substring(0, 50))
            .slice(0, 5);
    });
    if (imagesNoAlt.length > 0) {
        imagesNoAlt.forEach((img: string) => {
    ...
    const unlabeledButtons = await page.evaluate(() => {
        ...
        return buttons
            ...
            .map((btn: any) => btn.outerHTML.substring(0, 100))
            .slice(0, 5);
    });
    if (unlabeledButtons.length > 0) {
        unlabeledButtons.forEach((btn: string) => {
    ...
    if (unlabeledInputs.length > 0) {
        unlabeledInputs.forEach((inp: string) => {
