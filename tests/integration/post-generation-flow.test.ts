...
import { TIER_LIMITS, getTierLimits, getTierInfo, formatPrice } from '@/lib/tiers/config';
import { 
  canConnect, 
  canSchedule, 
  canGenerate, 
...
describe('Post Generation, Scheduling & Publishing Flow', () => {
  ...
  describe('Credit Validation', () => {
    ...
    it('should calculate credits correctly for multi-platform post', () => {
      expect(calculatePostCredits(['instagram', 'facebook', 'twitter'])).toBe(3);
    });
  ...
  describe('Credit Flow Scenarios', () => {
    ...
    it('should reserve credits when scheduling', () => {
      ...
      const canReserve = hasEnoughCredits(initialCredits, reserved, costToSchedule);
...
describe('Admin Dashboard Features', () => {
  it('should support multiple analytics tabs', () => {
    const tabs = ['users', 'revenue', 'transactions', 'analytics', 'jobs', 'errors', 'automation'];
