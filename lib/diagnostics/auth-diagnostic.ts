...
export async function diagnoseAuth() {
  ...
  console.log(
    `Trusted Origins: http://localhost:3000, https://purple-glow-social-2-0.vercel.app`
  );
  ...
  if (!process.env.BETTER_AUTH_SECRET) {
    ...
    return {
      ...
      solution:
        "Set BETTER_AUTH_SECRET environment variable (min 32 characters)",
    };
  }
  ...
  if (baseUrl !== publicUrl) {
    console.log(
      "⚠️  Warning: BETTER_AUTH_URL and NEXT_PUBLIC_BETTER_AUTH_URL differ"
    );
  ...
  if (isProduction) {
    ...
    if (baseUrl !== expectedUrl) {
      console.log(
        `⚠️  Warning: BETTER_AUTH_URL should be '${expectedUrl}' in production`
      );
    ...
    if (publicUrl !== expectedUrl) {
      console.log(
        `⚠️  Warning: NEXT_PUBLIC_BETTER_AUTH_URL should be '${expectedUrl}' in production`
      );
